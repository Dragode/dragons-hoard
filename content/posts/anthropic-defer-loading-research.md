+++
date = '2026-02-01T22:07:43+08:00'
draft = false
title = 'Anthropic defer_loading 深度调研报告'
tags = ['AI', 'Agent', 'MCP', 'Claude Code', 'API']
categories = ['技术调研']
+++

> 调研日期：2026-02-01

## 一、概述

`defer_loading` 是 Anthropic 于 **2025 年 11 月 24 日**发布的 **Advanced Tool Use（高级工具使用）** 特性之一，属于 Tool Search Tool 功能的核心组成部分。该特性通过延迟加载机制，让 Claude 能够动态发现和按需加载工具，而不是在对话开始时将所有工具定义塞入上下文窗口。

## 二、发布时间与版本信息

| 项目 | 信息 |
|------|------|
| **发布日期** | 2025 年 11 月 24 日 |
| **发布公告** | [Introducing advanced tool use on the Claude Developer Platform](https://www.anthropic.com/engineering/advanced-tool-use) |
| **Beta Header** | `advanced-tool-use-2025-11-20` |
| **Tool Search 版本** | `tool_search_tool_regex_20251119`（正则）<br>`tool_search_tool_bm25_20251119`（BM25） |
| **支持模型** | Claude Opus 4.5、Claude Sonnet 4.5 及以上 |
| **状态** | 公开 Beta |

## 三、出现背景：解决什么问题？

### 3.1 核心痛点：工具定义的上下文膨胀

随着 MCP（Model Context Protocol）和 AI Agent 的发展，实际生产环境中往往需要连接多个服务器、使用大量工具。Anthropic 在博客中给出了一个典型场景：

| MCP 服务器 | 工具数量 | Token 消耗 |
|------------|---------|-----------|
| GitHub | 35 | ~26K tokens |
| Slack | 11 | ~21K tokens |
| Sentry | 5 | ~3K tokens |
| Grafana | 5 | ~3K tokens |
| Splunk | 2 | ~2K tokens |
| **总计** | **58** | **~55K tokens** |

加上 Jira（~17K tokens），很快就会达到 **100K+ tokens** 的开销——**对话还没开始，上下文窗口就已经被工具定义占满了**。

Anthropic 内部测试显示，工具定义曾消耗高达 **134K tokens**。

### 3.2 次要痛点：工具选择准确性下降

当工具数量超过 **20-30 个**时，模型选择正确工具的能力会显著下降。特别是当工具名称相似时（如 `notification-send-user` vs `notification-send-channel`），错误选择和参数错误是最常见的失败模式。

### 3.3 问题总结

| 问题 | 影响 |
|------|------|
| **上下文膨胀** | 50 个工具 ≈ 10-20K tokens，严重挤占对话空间 |
| **选择准确性下降** | 30-50 个工具后，工具选择错误率显著上升 |
| **成本增加** | Token 消耗直接影响 API 调用成本 |
| **响应延迟** | 更大的上下文意味着更慢的推理速度 |

## 四、defer_loading 的解决方案

### 4.1 核心机制

```
传统方式：
┌─────────────────────────────────────────────────────┐
│ 上下文窗口                                           │
│ ┌─────────────────────────────────────────────────┐ │
│ │ 工具1定义 │ 工具2定义 │ ... │ 工具50定义 │ 对话  │ │
│ │  (~72K tokens 被工具定义占据)                    │ │
│ └─────────────────────────────────────────────────┘ │
└─────────────────────────────────────────────────────┘

使用 defer_loading：
┌─────────────────────────────────────────────────────┐
│ 上下文窗口                                           │
│ ┌─────────────────────────────────────────────────┐ │
│ │ Tool Search Tool │ 3-5个按需加载的工具 │ 对话空间 │ │
│ │    (~500 tokens)    (~3K tokens)    (大量可用)   │ │
│ └─────────────────────────────────────────────────┘ │
│                                                     │
│ 延迟加载池（不占用上下文）                            │
│ ┌─────────────────────────────────────────────────┐ │
│ │ 工具1 │ 工具2 │ ... │ 工具10000 (可扩展至万级)   │ │
│ └─────────────────────────────────────────────────┘ │
└─────────────────────────────────────────────────────┘
```

### 4.2 工作流程

1. **标记工具**：在工具定义中添加 `defer_loading: true`
2. **初始加载**：Claude 只看到 Tool Search Tool（~500 tokens）+ 非延迟工具
3. **动态搜索**：当 Claude 需要特定能力时，通过搜索发现相关工具
4. **按需扩展**：API 返回 3-5 个最相关的工具引用，自动展开为完整定义
5. **执行调用**：Claude 从发现的工具中选择并调用

### 4.3 代码示例

```python
# 基础用法
{
    "tools": [
        # Tool Search Tool（必须非延迟）
        {
            "type": "tool_search_tool_regex_20251119",
            "name": "tool_search_tool_regex"
        },
        # 标记为延迟加载的工具
        {
            "name": "github.createPullRequest",
            "description": "Create a pull request",
            "input_schema": {...},
            "defer_loading": True  # 关键标记
        },
        # ... 更多延迟加载的工具
    ]
}

# MCP 服务器整体配置
{
    "type": "mcp_toolset",
    "mcp_server_name": "google-drive",
    "default_config": {"defer_loading": True},  # 默认全部延迟
    "configs": {
        "search_files": {"defer_loading": False}  # 保留高频工具
    }
}
```

## 五、与传统工具调用的对比

### 5.1 性能对比

| 指标 | 传统方式 | 使用 defer_loading | 改善 |
|------|---------|-------------------|------|
| **初始 Token 消耗** | ~77K tokens | ~8.7K tokens | **↓ 85%** |
| **上下文保留** | ~5% 可用 | ~95% 可用 | **↑ 90%** |
| **最大工具数** | 30-50 个（实用） | **10,000 个** | **↑ 200x** |

### 5.2 准确性提升

| 模型 | 传统方式 | Tool Search | 提升 |
|------|---------|-------------|------|
| **Claude Opus 4** | 49% | 74% | **+25%** |
| **Claude Opus 4.5** | 79.5% | 88.1% | **+8.6%** |

### 5.3 功能对比

| 特性 | 传统工具调用 | defer_loading |
|------|------------|---------------|
| 工具发现 | 静态（全部预加载） | 动态（按需搜索） |
| Token 效率 | 低（全量加载） | 高（仅加载所需） |
| 扩展性 | 受限（30-50 工具） | 高（万级工具） |
| Prompt 缓存 | 完全兼容 | 兼容（延迟工具不影响缓存） |
| 搜索方式 | 无 | Regex / BM25 |
| 实现复杂度 | 简单 | 中等（需配置搜索工具） |

## 六、最佳实践

### 6.1 适用场景

**推荐使用 defer_loading：**
- 工具定义消耗 >10K tokens
- 可用工具数量 >10 个
- 构建 MCP 驱动的多服务器系统
- 遇到工具选择准确性问题

**不推荐使用：**
- 工具数量 <10 个
- 所有工具在每次会话中都频繁使用
- 工具定义非常紧凑

### 6.2 配置建议

1. **保留 3-5 个高频工具**为非延迟，确保常用操作零延迟
2. **优化工具名称和描述**，提升搜索匹配准确性
3. **在 System Prompt 中说明可用工具类别**，引导 Claude 搜索方向
4. **结合 Prompt Caching** 进一步优化多轮对话性能

## 七、局限性

| 限制 | 说明 |
|------|------|
| **模型支持** | 仅 Claude Sonnet 4.5+ 和 Opus 4.5+（不支持 Haiku） |
| **最大工具数** | 10,000 个 |
| **搜索返回** | 每次搜索返回 3-5 个工具 |
| **Regex 长度** | 最大 200 字符 |
| **框架支持** | 需直接使用 Anthropic SDK，LangChain 等框架暂未支持 |
| **不兼容 Tool Use Examples** | 两个特性不能同时使用 |
| **AWS Bedrock 限制** | 仅 InvokeModel API 支持，Converse API 不支持（详见下节） |

## 七点五、AWS Bedrock 兼容性问题

### 问题现象

使用 AWS Bedrock 作为 Claude Code 后端时，可能遇到以下错误：

```
502 {"error":"Failed to invoke AWS Bedrock: operation error Bedrock Runtime:
InvokeModelWithResponseStream, https response error StatusCode: 400,
RequestID: xxx, ValidationException: tools.21.custom.defer_loading:
Extra inputs are not permitted"}
```

### 根本原因

AWS Bedrock 对 `defer_loading` 的支持**取决于使用的 API 类型**：

| AWS Bedrock API | defer_loading 支持 | Beta Header |
|-----------------|-------------------|-------------|
| **InvokeModel / InvokeModelWithResponseStream** | ✅ 支持 | `tool-search-tool-2025-10-19` |
| **Converse / ConverseStream API** | ❌ 不支持 | - |

AWS 官方文档明确指出：
> "Note that this feature is currently only available via the **InvokeModel** and **InvokeModelWithResponseStream** APIs."

当 Claude Code 使用 Bedrock 的 **Converse API** 时，`defer_loading` 参数会被视为无效字段，触发 `ValidationException`。

### 解决方案

#### 方案 1：使用环境变量禁用（推荐）

在 `~/.claude/settings.json` 的 `env` 字段中添加：

```json
{
  "env": {
    "ENABLE_TOOL_SEARCH": "false"
  }
}
```

或在 shell 中设置：

```bash
export ENABLE_TOOL_SEARCH=false
```

这是**最可靠的解决方案**，经实际验证可以完全避免 `defer_loading` 参数被发送到 Bedrock。

`ENABLE_TOOL_SEARCH` 环境变量随 Claude Code **2.1.7 版本**（2026-01-14）与 MCP Tool Search 功能一起引入，与文章中的时间线完全吻合。

#### 方案 2：禁用所有实验性 Beta 功能

```bash
export CLAUDE_CODE_DISABLE_EXPERIMENTAL_BETAS=1
```

此方案会禁用所有实验性功能，可能影响其他 Beta 特性的使用。

#### 方案 3：切换到 InvokeModel API（适用于直接 API 调用）

如果你是直接调用 AWS Bedrock API（而非通过 Claude Code），可以：

1. 使用 `InvokeModel` 或 `InvokeModelWithResponseStream` API
2. 添加 Beta Header：`"anthropic_beta": ["tool-search-tool-2025-10-19"]`

```json
{
    "anthropic_version": "bedrock-2023-05-31",
    "anthropic_beta": ["tool-search-tool-2025-10-19"],
    "tools": [{
        "type": "tool_search_tool_regex",
        "name": "tool_search_tool_regex"
    }, {
        "name": "your_tool",
        "defer_loading": true,
        ...
    }]
}
```

### 各平台支持情况对比

| 平台 | defer_loading 支持 | 备注 |
|------|-------------------|------|
| **Claude API（直连）** | ✅ 完全支持 | 推荐方式 |
| **AWS Bedrock InvokeModel** | ✅ 支持 | 需添加 Beta Header |
| **AWS Bedrock Converse** | ❌ 不支持 | 会报 ValidationException |
| **Google Vertex AI** | ✅ 支持 | - |

### 配置验证

修改配置后，重启 Claude Code 并检查是否生效：

```bash
# 检查环境变量
echo $ENABLE_TOOL_SEARCH
echo $CLAUDE_CODE_DISABLE_EXPERIMENTAL_BETAS

# 启动 Claude Code
claude
```

如果配置正确，应该不再出现 `defer_loading: Extra inputs are not permitted` 错误。

## 八、总结

### 8.1 defer_loading 的本质

`defer_loading` 本质上是一种 **"工具目录化"** 策略：
- 将工具从"全部加载到内存"变为"存入可搜索的目录"
- Claude 在需要时"查字典"，而不是"全部记在脑子里"
- 类似于数据库的延迟加载/懒加载模式在 LLM 工具调用中的应用

### 8.2 核心价值

| 维度 | 价值 |
|------|------|
| **成本** | Token 消耗降低 85%，直接降低 API 成本 |
| **能力** | 支持万级工具，解锁复杂 Agent 场景 |
| **准确性** | 工具选择准确率提升 8-25% |
| **体验** | 更多上下文用于实际对话，响应更快更准 |

### 8.3 适合谁？

- 构建 **MCP 多服务器集成** 的开发者
- 开发 **企业级 AI Agent**（需连接多个 SaaS 系统）
- 任何遇到 **工具定义膨胀** 问题的 Claude API 用户

---

## 九、Claude Code 采用时间线

### 关键时间节点

| 日期 | 事件 | 版本 |
|------|------|------|
| **2025-11-24** | Anthropic 发布 Advanced Tool Use（含 defer_loading）API | - |
| **2025-12-01** | 社区提交 Feature Request（GitHub Issue #12836） | - |
| **2025-12-20** | 实验性标志 `ENABLE_EXPERIMENTAL_MCP_CLI` 可用（已弃用） | ~2.1.x |
| **2026-01-14** | Anthropic 官方宣布 Claude Code 支持 MCP Tool Search | 2.1.7 |
| **2026-01-14** | **MCP Tool Search auto mode 默认启用**，引入 `ENABLE_TOOL_SEARCH` 环境变量 | **2.1.7** |
| **2026-01-15** | 2.1.9 版本添加 `auto:N` 语法自定义阈值 | 2.1.9 |

**`ENABLE_TOOL_SEARCH` 环境变量**（2.1.7+ 引入）：

| 值 | 说明 |
|----|------|
| `auto` | 默认值，MCP 工具超过 10% 上下文时自动启用 |
| `auto:N` | 自定义阈值（2.1.9+），N 为百分比 |
| `true` | 强制启用 |
| `false` | 禁用（解决 AWS Bedrock 兼容性问题） |

### 详细发展过程

#### 阶段 1：API 发布与社区需求（2025-11 ~ 2025-12）

**2025-11-24**：Anthropic 发布 Advanced Tool Use API，包含：
- Tool Search Tool（`defer_loading: true`）
- Programmatic Tool Calling
- Tool Use Examples

**2025-12-01**：用户 `matthewod11-stack` 在 GitHub 提交 Issue #12836，请求 Claude Code 支持这些新特性。该 issue 获得 **104+ 个 👍**，成为"最受请求的功能之一"。

社区痛点描述（来自 Issue）：
> "Claude Code loads all tool definitions upfront at session start, which consumes significant context tokens - especially for users with multiple MCP servers, plugins, and agents configured."

#### 阶段 2：实验性支持（2025-12）

在正式发布前，Anthropic 提供了实验性标志：

```bash
ENABLE_EXPERIMENTAL_MCP_CLI=true
```

用户可以通过设置此环境变量提前体验 defer_loading 功能。

#### 阶段 3：正式发布（2026-01-14）

**2026-01-14**，Anthropic 技术人员 **Thariq Shihipar** 宣布：

> "Today we're rolling out MCP Tool Search for Claude Code. As MCP has grown to become a more popular protocol and agents have become more capable, we've found that MCP servers may have up to 50+ tools and take up a large amount of context. Tool Search allows Claude Code to dynamically load tools into context when MCP tools would otherwise take up a lot of context."

**版本 2.1.7** 的 Changelog 明确记录：

```
Enabled MCP tool search auto mode by default for all users.
When MCP tool descriptions exceed 10% of the context window,
they are automatically deferred and discovered via the MCPSearch
tool instead of being loaded upfront. This reduces context usage
for users with many MCP tools configured. Users can disable this
by adding `MCPSearch` to `disallowedTools` in their settings.
```

#### 阶段 4：持续优化（2026-01-15+）

**版本 2.1.9** 添加了更灵活的配置：

```
Added `auto:N` syntax for configuring the MCP tool search
auto-enable threshold, where N is the context window percentage (0-100)
```

这允许用户自定义触发 Tool Search 的阈值（默认 10%）。

### Claude Code 中的工作机制

```
启动时检测
    │
    ▼
MCP 工具描述占用 > 10% 上下文？
    │
    ├─ 否 → 传统方式：全部加载
    │
    └─ 是 → 启用 Tool Search
              │
              ├─ 标记工具为 defer_loading: true
              ├─ 注入 MCPSearch 工具
              └─ 按需发现和加载工具
```

### 配置方式

`ENABLE_TOOL_SEARCH` 环境变量在 **2.1.7 版本**（2026-01-14）正式引入，支持以下值：

| 值 | 说明 | 引入版本 |
|----|------|---------|
| `auto` | 默认值。当 MCP 工具超过 10% 上下文时自动启用 | 2.1.7 |
| `auto:N` | 自定义阈值，N 为上下文窗口百分比（0-100） | 2.1.9 |
| `true` | 强制启用 Tool Search | 2.1.7 |
| `false` | 禁用 Tool Search | 2.1.7 |

#### 自动模式（默认）
- 当 MCP 工具占用超过 10% 上下文时自动启用
- 无需任何配置

#### 禁用 Tool Search
```json
// settings.json
{
  "env": {
    "ENABLE_TOOL_SEARCH": "false"
  }
}
```

#### 自定义阈值（2.1.9+）
```json
// 设置为 5% 阈值
{
  "mcpToolSearch": "auto:5"
}
```

### 发布前后对比

#### 发布前用户反馈

**Scott Spence** 记录的实际数据：
- 启动 Claude Code 后，**66,000 tokens 立即消失**（占 200K 上下文的 33%）
- MCP 工具消耗 82K tokens（41% 上下文）
- 只剩 12K tokens（5.8%）的可用空间

**Sam McLeod** 的 MCP 服务器分析：

| MCP 服务器 | 工具数 | Token 消耗 |
|------------|--------|-----------|
| GitHub | 91 | ~46,000 |
| Playwright | 21 | ~9,700 |
| AWS Cost Explorer | 7 | ~9,100 |
| Docker | 135 | ~125,000 |

#### 发布后效果

**Token 节省**：从 ~134K 降至 ~5K（**85% 减少**）

**准确性提升**：
- Opus 4: 49% → 74%
- Opus 4.5: 79.5% → 88.1%

### 社区反响

**Simon Willison**（知名开发者）：
> "Context pollution is why I rarely used MCP, now that it's solved there's no reason not to hook up dozens or even hundreds of MCPs to Claude Code"

**Boris Cherny**（Claude Code 负责人）：
> "Every Claude Code user just got way more context, better instruction following, and the ability to plug in even more tools."

**VentureBeat** 评价：
> "The Claude Code team released an update that fundamentally alters this equation... It is a shift that moves AI agents from a brute-force architecture to something resembling modern software engineering."

### 时间线总结

| 问题 | 答案 |
|------|------|
| **API 何时发布** | 2025-11-24 |
| **Claude Code 何时支持** | 2026-01-14，版本 2.1.7 |
| **从 API 到 CLI 的延迟** | ~51 天 |
| **默认行为** | 自动启用（MCP 工具 > 10% 上下文时） |
| **可否禁用** | 可以，在 `env` 中设置 `ENABLE_TOOL_SEARCH=false` |

---

## 十、信息来源

| 来源 | 类型 | 日期 |
|------|------|------|
| [Anthropic Engineering Blog](https://www.anthropic.com/engineering/advanced-tool-use) | 官方公告 | 2025-11-24 |
| [Claude API Docs - Tool Search Tool](https://platform.claude.com/docs/en/agents-and-tools/tool-use/tool-search-tool) | 官方文档 | - |
| [Claude Code Changelog](https://code.claude.com/docs/en/changelog) | 官方文档 | - |
| [GitHub Issue #12836](https://github.com/anthropics/claude-code/issues/12836) | 社区讨论 | 2025-12-01 |
| [VentureBeat 报道](https://venturebeat.com/orchestration/claude-code-just-got-updated-with-one-of-the-most-requested-user-features) | 媒体报道 | 2026-01-15 |
| [Unified.to Blog](https://unified.to/blog/scaling_mcp_tools_with_anthropic_defer_loading) | 技术博客 | 2025-12-27 |
| [Arcade Blog](https://blog.arcade.dev/anthropic-tool-search-claude-mcp-runtime) | 技术分析 | 2025-12-02 |
| [atcyrus.com 深度解析](https://www.atcyrus.com/stories/mcp-tool-search-claude-code-context-pollution-guide) | 技术博客 | 2026-01-15 |
